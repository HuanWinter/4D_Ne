...
for i in range(num_init,num_init+num):
    ...
    # Read and precprocess RO data
    Out = project.Preprocess(X_temp.T,Y_temp.T)
    X_t = Out[0]
    Y_t = Out[1]
    ...
    # Normlisation
    X,Y = project.Normlise(X_t, Y_t, False)

    #ipdb.set_trace()
    Y = Y.unsqueeze(0)

    train , test = project.Dataset_create(X.T, Y.T, False)
...
########################## Modelling ##################################
    ...
    net = project.Modelling(len(index_X), 1, train, test)
    Print_test(net)
    time.sleep(.5)
